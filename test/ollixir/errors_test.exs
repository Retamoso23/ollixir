defmodule Ollixir.ErrorsTest do
  use Supertester.ExUnitFoundation, isolation: :basic

  alias Ollixir.{ConnectionError, Errors, RequestError, ResponseError}
  ...
  describe "format_for_log/1" do
    test "formats RequestError" do
      error = RequestError.exception("bad")
      assert Errors.format_for_log(error) == "[Ollixir.RequestError] bad"
    end

    test "formats ResponseError" do
      error = ResponseError.exception(status: 500, body: %{"error" => "boom"})
      assert Errors.format_for_log(error) == "[Ollixir.ResponseError] HTTP 500: boom"
    end

    test "formats ConnectionError" do
      error = ConnectionError.exception(reason: :timeout)
      assert String.contains?(Errors.format_for_log(error), "[Ollixir.ConnectionError]")
    end
  end
end
